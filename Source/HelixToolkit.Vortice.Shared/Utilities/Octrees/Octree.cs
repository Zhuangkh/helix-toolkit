...
#endif
{
    ...
    public abstract class DynamicOctreeBase<T> : IDynamicOctree
    {
        ...
        public float MIN_SIZE
        {
            get
            {
                return Parameter.MinOctantSize;
            }
        }
        /// <summary>
        ...
        {
            if (Bound.Max == Bound.Min || !CheckDimension())
            {
                treeBuilt = false;
                return;
            }
        ...
        public static BoundingBox[] CreateOctants(ref BoundingBox box, float minSize)
        {
            ...
            var center = box.Min + half;
            var Min = box.Min;
            var Max = box.Max;
            //Create subdivided regions for each octant
            return new BoundingBox[8] {
                new BoundingBox(Min, center),
                new BoundingBox(new Vector3(center.X, Min.Y, Min.Z), new Vector3(Max.X, center.Y, center.Z)),
                new BoundingBox(new Vector3(center.X, Min.Y, center.Z), new Vector3(Max.X, center.Y, Max.Z)),
                new BoundingBox(new Vector3(Min.X, Min.Y, center.Z), new Vector3(center.X, center.Y, Max.Z)),
                new BoundingBox(new Vector3(Min.X, center.Y, Min.Z), new Vector3(center.X, Max.Y, center.Z)),
                new BoundingBox(new Vector3(center.X, center.Y, Min.Z), new Vector3(Max.X, Max.Y, center.Z)),
                new BoundingBox(center, Max),
                new BoundingBox(new Vector3(Min.X, center.Y, center.Z), new Vector3(center.X, Max.Y, Max.Z))
                };
        }
        /// <summary>
        /// 
        ...
        {
            var dimensions = Bound.Max - Bound.Min;

            if (dimensions == Vector3.Zero)
            {
                Bound = FindEnclosingBox();
            }
            dimensions = Bound.Max - Bound.Min;
            //Check to see if the dimensions of the box are greater than the Min dimensions
        ...
        {
            var v = (bound.Max - bound.Min) / 2 + bound.Min;
            bound = new BoundingBox(bound.Min - v, bound.Max - v);
            var max = Math.Max(bound.Max.X, Math.Max(bound.Max.Y, bound.Max.Z));
            ...
            ref List<HitTestResult> hits, float hitThickness)
        {
            ...
            if (modelInv == default(Matrix))
            {
                return false;
            }//Cannot be inverted
            var rayWS = context.RayWS;
            var rayModel = new Ray(Vector3.Transform(rayWS.Position, modelInv), Vector3.Normalize(Vector3.TransformNormal(rayWS.Direction, modelInv)));
        ...
        /// <returns></returns>
        public virtual bool FindNearestPointBySphere(HitTestContext context, ref global::Vortice.Mathematics.BoundingSphere sphere, ref List<HitTestResult> points)
        {
            if (points == null)
            {
        ...
        public virtual bool FindNearestPointFromPoint(HitTestContext context, ref Vector3 point, ref List<HitTestResult> results, float heuristicSearchFactor = 1f)
        {
            ...
            var sphere = new global::Vortice.Mathematics.BoundingSphere(point, float.MaxValue);
            var isIntersect = false;
            var isHit = false;
        ...
        /// <param name="isIntersect"></param>
        /// <returns></returns>
        public abstract bool FindNearestPointBySphereExcludeChild(HitTestContext context, ref global::Vortice.Mathematics.BoundingSphere sphere,
        ...
        {
            return source.Contains(target) == ContainmentType.Contains;
        }
        /// <summary>
        /// Return new root
        /// </summary>
        ...
        public static IDynamicOctree Expand(IDynamicOctree oldRoot, ref Vector3 direction, CreateNodeDelegate createNodeFunc)
        {
            ...
            var dimension = rootBound.Max - rootBound.Min;
            var half = dimension / 2 + epsilon;
            var center = rootBound.Min + half;
            ...
            bound = BoundingBox.CreateMerged(rootBound, bound);
            ...
            if (!oldRoot.IsEmpty)
            {
                ...
                {
                    var d = (newRoot.Octants[i].Min - rootBound.Min).LengthSquared();
                    if (d < diff)
                    {
                        diff = d;
                        idx = i;
        ...
        {
            var sphere = new global::Vortice.Mathematics.BoundingSphere(point, radius);
    ...
    {
        private float MinOctantSize = 1f;
        /// <summary>
        /// Min Octant size.
        /// </summary>
        public float MinOctantSize
        {
            set
            {
                Set(ref MinOctantSize, value);
            }
            get
            {
                return MinOctantSize;
            }
        }

        private int minObjectSizeToSplit = 2;
        /// <summary>
        /// Min object in each octant to start splitting into smaller octant during build
        /// </summary>
        ...
        {
            MinOctantSize = Math.Max(0, minSize);
        }
        /// <summary>
        /// 
        /// </summary>
        ...
        : DynamicOctreeBase<KeyValuePair<int, BoundingBox>>
    {
                ...
                for (var i = 0; i < Objects.Count; ++i)
                {
                    ...
                    if (Collision.RayIntersectsTriangle(ref rayModel, ref v0, ref v1, ref v2, out d))
                    {
                        if (d >= 0 && d < result.Distance) // If d is NaN, the condition is false.
                        {
                            ...
                            var pointWorld = Vector3.Transform(rayModel.Position + (rayModel.Direction * d), modelMatrix);
                            result.PointHit = pointWorld;
                            result.Distance = (rayWS.Position - pointWorld).Length();

                            var p0 = Vector3.Transform(v0, modelMatrix);
                            var p1 = Vector3.Transform(v1, modelMatrix);
                            var p2 = Vector3.Transform(v2, modelMatrix);
        ...
        /// <param name="isIntersect"></param>
        /// <returns></returns>
        public override bool FindNearestPointBySphereExcludeChild(HitTestContext context, ref global::Vortice.Mathematics.BoundingSphere sphere,
            ref List<HitTestResult> result, ref bool isIntersect)
        {
            var isHit = false;
            var containment = Bound.Contains(sphere);
    ...
    public class LineGeometryOctree : DynamicOctreeBase<KeyValuePair<int, BoundingBox>>
    {
            ...
            ref Ray rayModel, ref List<HitTestResult> hits, ref bool isIntersect, float hitThickness)
        {
            ...
            var bound = Bound;
            bound.Max += new Vector3(hitThickness);
            bound.Min -= new Vector3(hitThickness);
            ...
            if (rayModel.Intersects(bound).HasValue)
            {
                ...
                for (var i = 0; i < Objects.Count; ++i)
                {
                    ...
                    var t0 = Vector3.Transform(v0, modelMatrix);
                    var t1 = Vector3.Transform(v1, modelMatrix);
                    Vector3 sp, tp;
                    float sc, tc;
    ...
    public class PointGeometryOctree : DynamicOctreeBase<int>
    {
            ...
            ref Ray rayModel, ref List<HitTestResult> hits, ref bool isIntersect, float hitThickness)
        {
            ...
            if (rayModel.Intersects(bound).HasValue)
            {
                ...
                var clickPoint = Vector3.Transform(clickPoint3, svpm);
                pos3 = Vector3.Transform(pos3, svpm);
        ...
        /// <summary>
        /// <see cref="DynamicOctreeBase{T}.FindNearestPointBySphereExcludeChild(HitTestContext, ref global::Vortice.Mathematics.BoundingSphere, ref List{HitTestResult}, ref bool)"/>
        /// </summary>
        /// <param name="context"></param>
        /// <param name="sphere"></param>
        /// <param name="result"></param>
    ...
    public class InstancingModel3DOctree : DynamicOctreeBase<KeyValuePair<int, BoundingBox>>
    {
            ...
            : base(ref geometryBound, null, parameter, stackCache)
        {
            ...
            for (var i = 0; i < instanceMatrix.Count; ++i)
            {
                ...
                totalBound = BoundingBox.CreateMerged(totalBound, b);
                ++counter;
            }
            ...
            ref Ray rayModel, ref List<HitTestResult> hits, ref bool isIntersect, float hitThickness)
        {
            ...
            var rayWS = context.RayWS;
            if (rayWS.Intersects(bound).HasValue)
            {
                isIntersect = true;
                for (var i = 0; i < this.Objects.Count; ++i)
                {
                    ...
                    if (b.Intersects(rayWS).HasValue)
                    {
                        var result = new HitTestResult()
                        {
                            Tag = Objects[i].Key
