        ...
        public abstract class StaticOctree<T> : IOctreeBasic where T : unmanaged
        {
            ...
            public LineGeometry3D CreateOctreeLineModel()
            {
                ...
                for (var i = 0; i < octants.Count; ++i)
                {
                    ...
                    verts[0] = box.Min;
                    verts[1] = new Vector3(box.Min.X, box.Min.Y, box.Max.Z); //Z
                    verts[2] = new Vector3(box.Min.X, box.Max.Y, box.Min.Z); //Y
                    verts[3] = new Vector3(box.Max.X, box.Min.Y, box.Min.Z); //X

                    verts[7] = box.Max;
                    verts[4] = new Vector3(box.Max.X, box.Max.Y, box.Min.Z); //Z
                    verts[5] = new Vector3(box.Max.X, box.Min.Y, box.Max.Z); //Y
                    verts[6] = new Vector3(box.Min.X, box.Max.Y, box.Max.Z); //X
            ...
            protected static bool BoxContainsBox(ref BoundingBox source, ref BoundingBox target)
            {
                ...
                return source.Min.X <= target.Min.X && (target.Max.X <= source.Max.X &&
                    source.Min.Y <= target.Min.Y && target.Max.Y <= source.Max.Y) &&
                    source.Min.Z <= target.Min.Z && target.Max.Z <= source.Max.Z;
            }

            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            protected static bool BoxDisjointSphere(BoundingBox box, ref BoundingSphere sphere)
            {
                Vector3.Clamp(ref sphere.Center, ref box.Min, ref box.Maximum, out var vector);
