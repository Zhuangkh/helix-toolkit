...
using FontWeights = Vortice.DirectWrite.FontWeight;
using Thickness = HelixToolkit.Vortice.Core.Model.Scene2D.Thickness;
    ...
    public class BillboardSingleText3D : BillboardBase
    {
            ...
            object originalSource, bool fixedSize)
        {
            ...
            if (!IsInitialized || context == null || Width == 0 || Height == 0
                || (!fixedSize && !BoundingSphere.TransformBoundingSphere(modelMatrix).Intersects(rayWS).HasValue))
            {
                return false;
            }
