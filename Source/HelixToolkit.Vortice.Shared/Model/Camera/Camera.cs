...
#endif
{
    namespace Cameras
    {
        ...
        public abstract class ProjectionCameraCore : CameraCore
        {
            ...
            {
                return CreateLeftHandSystem ? MatrixExtensions.LookAtLH(this.Position, this.Position + this.LookDirection, this.UpDirection)
                    : MatrixExtensions.LookAtRH(this.Position, this.Position + this.LookDirection, this.UpDirection);
            }
        ...
        public class OrthographicCameraCore : ProjectionCameraCore
        {
            ...
            public override Matrix CreateProjectionMatrix(float aspectRatio, float nearPlane, float farPlane)
            {
                return this.CreateLeftHandSystem ?
                    MatrixExtensions.OrthoLH(this.Width, (float)(this.Width / aspectRatio), nearPlane, Math.Min(1e15f, farPlane))
                    : MatrixExtensions.OrthoRH(this.Width, (float)(this.Width / aspectRatio), nearPlane, Math.Min(1e15f, farPlane));
            }
        ...
        public class PerspectiveCameraCore : ProjectionCameraCore
        {
            ...
            public override Matrix CreateProjectionMatrix(float aspectRatio, float nearPlane, float farPlane)
            {
                ...
                {
                    projM = MatrixExtensions.PerspectiveFovLH((float)fov, aspectRatio, nearPlane, farPlane);
                }
                else
                {
                    projM = MatrixExtensions.PerspectiveFovRH((float)fov, (float)aspectRatio, nearPlane, farPlane);
                }
